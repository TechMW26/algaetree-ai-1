...
function DashboardPageContent() {
  ...
  const commitNutrition = async () => {
    const sent = await sendChangeCommand(8, 0, {
      NutritionDosing: nutritionDraftRef.current,
    });
  ...
  const commitBubblesTiming = async (key: "on" | "off") => {
    const next = uiAirBubblesTimingRef.current;
    const sent = await sendChangeCommand(7, key === "on" ? next.on : next.off, {
      Operations: {
        ...uiOperationsRef.current,
        AirBubblesTiming: {
          On: next.on,
          Off: next.off,
        },
...
const dashboardHeaderFadeStyle: React.CSSProperties = {
  ...
  zIndex: 8,
  pointerEvents: "none",
  background: "linear-gradient(180deg, var(--bg) 0%, color-mix(in srgb, var(--bg) 70%, transparent) 48%, transparent 100%)",
